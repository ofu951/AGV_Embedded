...
#include <stdio.h>  // sprintf fonksiyonu için gerekli
#include <string.h>
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  // 2. UART KOMUT DİNLEYİCİSİ (Heartbeat ve Motor Komutları)
	  uint8_t rx_header[2];
	  if (HAL_UART_Receive(&huart1, rx_header, 2, 10) == HAL_OK)
	  {
		  ...
		  // B. Motor Komutları Paketi (AA 55)
			  ...
			  if (HAL_UART_Receive(&huart1, rx_payload, 4, 20) == HAL_OK) {
				  ...
				  int16_t value = (rx_payload[2] << 8) | rx_payload[1]; // Hız veya Adım değeri

				  if (msg_id == 0x03) {
					  // DC Motor Komutu (PWM)
					  agv_packet.dc_speed = value; // Telemetriye kaydet
